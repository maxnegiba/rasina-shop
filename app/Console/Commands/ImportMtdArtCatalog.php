...
final class ImportMtdArtCatalog extends Command
{
    protected $signature = 'mtd:import-catalog
        {source=storage/app/imports/mtd-art-final : Directorul catalogului final}
        {--publish-products : Publică produsele cu preț și stoc pozitiv}
        {--publish-posts : Publică articolele imediat}
        {--use-existing-posts : Nu creează și nu actualizează articole; folosește doar articolele existente pentru legături}
        {--skip-missing-posts : Continuă importul dacă un articol existent nu este găsit și lasă produsul fără legătură}
        ...
        {--preserve-existing-prices : Nu suprascrie prețurile produselor existente}
    ...
    protected $description = 'Importă catalogul final MTD ART: 4 categorii, RO/EN, SEO, WebP, alt text, galerie ordonată și articole asociate.';
    ...
    public function handle(): int
    {
        try {
            ...
            $catalog = $this->readJson($root . '/catalog-final.json');
            $articleFile = $this->readJson($root . '/articles-final.json');
            ...
            $result = DB::transaction(function () use ($categories, $products, $articles, $staged): array {
                ...
                $productResult = $this->importProducts($products, $staged, $categoryMap, $postResult['bySlug']);
    ...
    {
        if ((int) ($catalog['schema_version'] ?? 0) !== 3) {
            throw new RuntimeException('Catalogul trebuie să folosească schema_version=3.');
        }

        $expectedCategories = ['cruci', 'baticuri', 'butoni', 'obiecte-de-decor'];
        ...
        $actualCategories = array_keys($categorySlugs);
        sort($actualCategories);
        sort($expectedCategories);
        if ($actualCategories !== $expectedCategories) {
            throw new RuntimeException('Categoriile trebuie să fie: cruci, baticuri, butoni și obiecte-de-decor.');
        }

        $types = (array) ($catalog['product_types'] ?? []);
        $expectedTypes = ['cruce', 'batic_scurt', 'batic_lung', 'buton', 'scrumiera_zimtata', 'platou_rotund', 'tava_alungita'];
        foreach ($expectedTypes as $type) {
            if (! isset($types[$type]['category_slug'], $types[$type]['price_ron'])) {
                throw new RuntimeException("Tip comercial lipsă sau incomplet: {$type}");
            }
        }
        ...
        foreach ($products as $index => $product) {
            foreach (['code', 'slug', 'category_slug', 'product_type', 'name', 'description_html', 'price_ron', 'image_folder', 'images', 'seo'] as $field) {
            ...
            if ($code === '' || isset($productCodes[$code])) {
                throw new RuntimeException("Cod de produs gol sau duplicat: {$code}");
            }
            if ($slug === '' || isset($productSlugs[$slug])) {
                throw new RuntimeException("Slug de produs gol sau duplicat: {$slug}");
            }
            if (! isset($types[$type])) {
                throw new RuntimeException("Tip comercial necunoscut pentru {$code}: {$type}");
            }
            if ((string) $types[$type]['category_slug'] !== (string) $product['category_slug']) {
                throw new RuntimeException("Categoria nu corespunde tipului comercial pentru {$code}.");
            }
            ...
            if ((float) $product['price_ron'] !== (float) $types[$type]['price_ron']) {
                throw new RuntimeException("Prețul produsului {$code} nu respectă schema tipului {$type}.");
            }
            ...
            foreach ((array) $product['images'] as $image) {
                foreach (['file', 'featured', 'sort_order', 'alt', 'sha256'] as $field) {
                ...
                if (! is_string($actualHash) || ! hash_equals(strtolower((string) $image['sha256']), strtolower($actualHash))) {
                    throw new RuntimeException("Checksum SHA-256 invalid pentru {$source}");
                }
                ...
                if ($order < 1 || isset($sortOrders[$order])) {
                    throw new RuntimeException("Ordine de galerie invalidă pentru {$slug}: {$order}");
                }
            ...
            $orders = array_keys($sortOrders);
            sort($orders);
            if ($orders !== range(1, count($orders))) {
                throw new RuntimeException("Galeria {$slug} trebuie să aibă ordine continuă de la 1.");
            }
        }
        ...
        foreach ($products as $product) {
            $related = trim((string) ($product['related_article_slug'] ?? ''));
            if ($related !== '' && ! isset($articleSlugs[$related])) {
                throw new RuntimeException("Produsul {$product['code']} indică un articol inexistent: {$related}");
            }
        }
    }
    private function printPlan(array $categories, array $products, array $articles, array $staged): void
    {
        ...
        $this->table(['Element', 'Număr'], [
            ['Categorii principale', count($categories)],
            ['Produse', count($products)],
            ['Cruci / CR', $typesCount['cruce'] ?? 0],
            ['Baticuri scurte / BS', $typesCount['batic_scurt'] ?? 0],
            ['Baticuri lungi / BL', $typesCount['batic_lung'] ?? 0],
            ['Butoni / BT', $typesCount['buton'] ?? 0],
            ['Scrumiere zimțate / OD-SZ', $typesCount['scrumiera_zimtata'] ?? 0],
            ['Platouri rotunde / OD-PR', $typesCount['platou_rotund'] ?? 0],
            ['Tăvi alungite / OD-PA', $typesCount['tava_alungita'] ?? 0],
            ['Articole RO/EN', count($articles)],
            ['Imagini verificate și convertite WebP', array_sum(array_map('count', $staged))],
        ]);
    }
    private function stageImages(string $root, array $products): array
    {
        ...
        foreach ($products as $product) {
            ...
            $codeFolder = strtolower(str_replace(' ', '-', (string) $product['code']));
            foreach ((array) $product['images'] as $image) {
                $source = $root . '/' . trim((string) $product['image_folder'], '/') . '/' . basename((string) $image['file']);
                $base = pathinfo((string) $image['file'], PATHINFO_FILENAME);
                $filename = $base . '.webp';
                $stagePath = $this->stageRoot . '/webp/' . $product['category_slug'] . '/' . $codeFolder . '-' . $slug . '/' . $filename;
                $destination = 'products/' . $product['category_slug'] . '/' . $codeFolder . '-' . $slug . '/' . $filename;
    ...
    {
        $required = [
            'products' => ['product_code', 'product_type', 'related_post_id', 'seo_translations'],
    ...
    private function importPosts(array $articles, array $staged): array
    {
        ...
        foreach ($articles as $article) {
            ...
            if ($this->option('use-existing-posts')) {
                if ($post === null) {
                    $message = "Articolul existent nu a fost găsit după slug: {$slug}.";

                    if ($this->option('skip-missing-posts')) {
                        $this->warnings[] = $message . ' Produsul va fi importat fără articol asociat.';
                        continue;
                    }

                    throw new RuntimeException(
                        $message . ' Corectează slugul articolului manual sau rulează și cu --skip-missing-posts.'
                    );
                }

                $bySlug[$slug] = $post;
                $this->line("articol existent folosit fără modificare: {$slug}");
                continue;
            }
    ...
    private function importProducts(array $products, array $staged, array $categoryMap, array $postBySlug): array
    {
        ...
        foreach ($products as $data) {
            ...
            $productByCode = Product::query()->where('product_code', $code)->first();
            $productBySlug = Product::query()->where('slug', $slug)->first();
            if ($productByCode !== null && $productBySlug !== null && ! $productByCode->is($productBySlug)) {
                throw new RuntimeException("Conflict de identificare: codul {$code} și slugul {$slug} aparțin unor produse diferite.");
            }
            $product = $productByCode ?? $productBySlug;
            ...
            $product->forceFill([
                ...
                'category_id' => $categoryMap[(string) $data['category_slug']]->getKey(),
                'related_post_id' => $relatedPost?->getKey(),
                'slug' => $slug,
                'price' => ($wasExisting && $this->option('preserve-existing-prices'))
                    ? $product->price
                    : round((float) $data['price_ron'], 2),
                'stock' => $stock,
            ...
            foreach ($staged[$slug] as $image) {
                ...
                $record = $product->images()->where('image_path', (string) $image['destination'])->first();
                $record ??= $product->images()->make();
                $record->forceFill([
            ...
            $wasExisting ? $updated++ : $created++;
            $this->line(($wasExisting ? 'actualizat' : 'creat') . ": {$code} / {$slug}");
        }
    ...
    private function createBackup(array $products, array $articles): string
    {
        ...
        $productCodes = array_values(array_filter(array_column($products, 'code')));
        $articleSlugs = array_values(array_filter(array_column($articles, 'slug')));
        $payload = [
            'created_at' => now()->toIso8601String(),
            'products' => Product::query()->where(function ($query) use ($productSlugs, $productCodes): void {
                $query->whereIn('slug', $productSlugs)->orWhereIn('product_code', $productCodes);
            })->with(['images', 'seo'])->get()->toArray(),
            'posts' => Post::query()->whereIn('slug', $articleSlugs)->with('seo')->get()->toArray(),
        ];
        $path = 'import-backups/mtd-art-final-' . now()->format('Ymd-His') . '.json';
