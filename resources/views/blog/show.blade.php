...
<div class="bg-ivory pt-12 pb-24">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Breadcrumbs -->
        <nav class="flex text-xs font-sans tracking-widest text-dark-brown/40 uppercase mb-8" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-2">
                ...
                <li><span class="mx-1">/</span></li>
                <li aria-current="page" class="text-dark-brown truncate max-w-[150px] sm:max-w-[300px]">
                    {{ $post->title }}
                </li>
            </ol>
        </nav>
        ...
        <header class="text-center mb-12">
            ...
            <h1 class="text-4xl sm:text-5xl md:text-6xl font-serif text-dark-brown mb-6 leading-tight">
                {{ $post->title }}
            </h1>

            <div class="flex items-center justify-center space-x-4 text-xs font-sans tracking-widest text-dark-brown/60 uppercase">
        ...
        <!-- Conținut Articol -->
        <article class="prose prose-stone lg:prose-lg max-w-none mx-auto prose-headings:font-serif prose-headings:font-normal prose-headings:text-dark-brown prose-p:font-light prose-p:text-dark-brown/80 prose-p:leading-relaxed prose-a:text-vintage-gold prose-a:no-underline hover:prose-a:underline prose-img:rounded-sm">
            {!! $post->content !!}
        </article>
        ...
        <div class="mt-16 pt-8 border-t border-black/10 flex flex-col sm:flex-row justify-between items-center gap-6">
            <div class="text-xs font-sans tracking-widest text-dark-brown/60 uppercase">
                <span>Distribuiți această poveste</span>
            </div>
            <div class="flex space-x-4">
                <button class="w-10 h-10 rounded-full border border-black/10 flex items-center justify-center text-dark-brown hover:border-vintage-gold hover:text-vintage-gold transition-colors">
                    F
                </button>
                <button class="w-10 h-10 rounded-full border border-black/10 flex items-center justify-center text-dark-brown hover:border-vintage-gold hover:text-vintage-gold transition-colors">
                    T
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            @foreach($recentPosts as $recentPost)
                <a href="{{ route('blog.show', $recentPost->slug) }}" class="group block bg-ivory shadow-sm border border-black/5 hover:border-vintage-gold/30 transition-all duration-300 hover:-translate-y-1">
                    ...
                    <div class="p-6">
                        <h3 class="font-serif text-xl text-dark-brown mb-3 line-clamp-2 group-hover:text-vintage-gold transition-colors">
                            {{ $recentPost->title }}
                        </h3>
                        <p class="text-sm font-light text-dark-brown/60 line-clamp-3 mb-4">
                            {{ strip_tags(substr($recentPost->content, 0, 150)) }}...
                        </p>
                        <span class="inline-flex items-center gap-2 text-[10px] font-sans tracking-[0.2em] text-dark-brown uppercase group-hover:text-vintage-gold transition-colors">
