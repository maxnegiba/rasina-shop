...
@section('content')
    <section class="relative h-[80vh] flex items-center justify-center overflow-hidden">
        <div class="absolute inset-0 bg-smoked-black/30 z-10"></div>
        ...
        <div class="relative z-20 text-center px-4">
            ...
            <a href="{{ route('shop.index') }}" 
               class="inline-block border border-ivory text-ivory px-10 py-4 uppercase tracking-widest text-sm hover:bg-ivory hover:text-smoked-black transition duration-500">
                Explorează Galeria
            </a>
        </div>
    </section>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-12">
            @foreach($featuredCategories as $category)
                <div class="group cursor-pointer">
                    ...
                    <div class="w-10 h-px bg-vintage-gold mx-auto mt-4 group-hover:w-20 transition-all"></div>
                </div>
            @endforeach
        </div>
    </section>
    ...
    <section class="bg-white py-24">
        <div class="max-w-7xl mx-auto px-4">
            <div class="flex justify-between items-end mb-12">
                <div>
                    <span class="text-vintage-gold uppercase tracking-widest text-xs">Colecția Nouă</span>
                    <h2 class="font-serif text-4xl mt-2">Piese Recente</h2>
                </div>
                <a href="{{ route('shop.index') }}" class="text-sm uppercase tracking-widest border-b border-vintage-gold pb-1">Vezi tot</a>
            </div>
                        ...
                        <a href="{{ route('shop.show', $product->slug) }}">
                            <div class="aspect-square overflow-hidden bg-ivory mb-4 relative">
                                ...
                                @if($product->is_custom)
                                    <span class="absolute top-4 left-4 bg-vintage-gold text-ivory text-[10px] px-2 py-1 uppercase tracking-tighter">La Comandă</span>
                                @endif
                            </div>
                            <h4 class="font-medium text-smoked-black uppercase text-sm tracking-wide">{{ $product->name }}</h4>
                            <p class="text-vintage-gold mt-1">
