...
@php
    $onlinePaymentButtonLabel = 'Plătește '.number_format($totalAmount, 2, ',', '.').' RON';
    $cashOnDeliveryButtonLabel = 'Plasează comanda';
@endphp
...
<div class="bg-ivory min-h-screen py-12 md:py-20">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid lg:grid-cols-[minmax(0,1fr)_360px] gap-7 lg:gap-10 items-start">
            <section class="bg-white p-5 sm:p-8 border border-black/10 shadow-sm" aria-labelledby="contact-heading">
                <form id="payment-form" class="space-y-8" novalidate>
                    <div>
                        <h2 id="contact-heading" class="font-serif text-2xl text-dark-brown border-b border-black/10 pb-3 mb-5">Date de contact</h2>
                        <label for="customer-email" class="block text-[10px] uppercase tracking-[0.14em] text-dark-brown/60 mb-2">Adresă de email</label>
                        <input id="customer-email" type="email" inputmode="email" autocomplete="email" required aria-describedby="email-help email-error" class="w-full border border-black/20 px-4 py-3.5 text-dark-brown focus:border-vintage-gold focus:ring-vintage-gold" placeholder="nume@exemplu.ro">
                    ...
                    <div>
                        <div class="flex items-start justify-between gap-4 border-b border-black/10 pb-3 mb-5">
                            <div>
                                ...
                                <p class="mt-1 text-xs text-dark-brown/50">Alegeți plata online sau ramburs la curier.</p>
                            </div>
                        ...
                        <div class="grid sm:grid-cols-2 gap-3 mb-5" role="group" aria-label="Alegeți metoda de plată">
                            <button id="method-online" type="button" aria-pressed="true" class="payment-method-toggle border-2 border-vintage-gold bg-vintage-gold/5 p-4 text-left transition-colors">
                                <span class="block text-sm font-semibold text-dark-brown">Plată online</span>
                                <span class="block mt-1 text-xs text-dark-brown/55">Card, Google Pay, Apple Pay și alte opțiuni disponibile.</span>
                            </button>
                            <button id="method-cod" type="button" aria-pressed="false" class="payment-method-toggle border border-black/15 bg-white p-4 text-left transition-colors hover:border-vintage-gold/60">
                                <span class="block text-sm font-semibold text-dark-brown">Ramburs la curier</span>
                                <span class="block mt-1 text-xs text-dark-brown/55">Achitați produsele la livrare.</span>
                            </button>
                        </div>

                        <div id="online-payment-panel">
                        ...
                        <div id="cod-payment-panel" class="hidden border border-vintage-gold/35 bg-vintage-gold/[0.06] p-5" aria-live="polite">
                            <p class="text-sm font-semibold text-dark-brown">Plată ramburs la curier</p>
                            <p class="mt-2 text-xs leading-relaxed text-dark-brown/65">Comanda este înregistrată fără plată online. Produsele se achită curierului la livrare. Livrarea nu este inclusă în preț.</p>
                        </div>
                    </div>
                    ...
                    <fieldset class="border-t border-black/10 pt-6 space-y-4">
                        ...
                        <div>
                            <label class="flex items-start gap-3 text-sm text-dark-brown/75 cursor-pointer">
                                ...
                                <span>Confirm că am citit <a href="{{ route('page.show', 'politica-de-confidentialitate') }}" target="_blank" rel="noopener" class="text-vintage-gold underline hover:text-dark-brown">Politica de Confidențialitate</a>.</span>
                            </label>
                    ...
                    <button id="submit" type="submit" class="w-full min-h-14 bg-dark-brown text-white px-5 py-4 uppercase tracking-[0.16em] text-[10px] font-semibold hover:bg-vintage-gold transition-colors shadow-sm flex justify-center items-center disabled:opacity-60 disabled:cursor-wait">
                        <span id="button-text">{{ $onlinePaymentButtonLabel }}</span>
            ...
            <aside class="bg-white p-5 sm:p-6 border border-black/10 shadow-sm lg:sticky lg:top-32" aria-labelledby="order-summary-heading">
                ...
                <div class="mt-7 pt-5 border-t border-black/5 space-y-3 text-[10px] text-dark-brown/55 leading-relaxed">
                    <p>✓ Pentru plata online, datele cardului sunt procesate securizat de Stripe.</p>
                    <p>✓ Produsele sunt rezervate cât finalizați comanda.</p>
                    <p>✓ Confirmarea comenzii și documentul proforma sunt trimise pe email după plasarea comenzii.</p>
...
(() => {
    ...
    const elements = stripe.elements({
        ...
        appearance: {
            theme: 'stripe',
            variables: {
                colorPrimary: '#CFB53B', colorBackground: '#ffffff', colorText: '#2C1E16', colorDanger: '#b42318',
                fontFamily: 'Manrope, ui-sans-serif, system-ui, sans-serif', spacingUnit: '4px', borderRadius: '0px',
            },
        },
    });

    const addressElement = elements.create('address', {
        mode: 'shipping', allowedCountries: ['RO'], fields: {phone: 'always'}, validation: {phone: {required: 'always'}},
    });
    ...
    const onlineButton = document.getElementById('method-online');
    const codButton = document.getElementById('method-cod');
    ...
    const onlinePaymentButtonLabel = {{ Illuminate\Support\Js::from($onlinePaymentButtonLabel) }};
    const cashOnDeliveryButtonLabel = {{ Illuminate\Support\Js::from($cashOnDeliveryButtonLabel) }};
    let checkoutMethod = 'online';
    let isSubmitting = false;

    try { emailInput.value = window.sessionStorage.getItem(storageKey) || ''; } catch (_) {}
    emailInput.addEventListener('input', () => { try { window.sessionStorage.setItem(storageKey, emailInput.value.trim()); } catch (_) {} });

    paymentElement.on('ready', () => {
        document.getElementById('payment-loading')?.classList.add('hidden');
        document.getElementById('payment-element')?.classList.remove('hidden');
    });
    paymentElement.on('loaderror', () => showGlobalError('Metodele de plată nu s-au încărcat. Verificați conexiunea și reîncărcați pagina.'));
    ...
    function selectMethod(method) {
        ...
        codPanel.classList.toggle('hidden', isOnline);
        onlineButton.setAttribute('aria-pressed', String(isOnline));
        codButton.setAttribute('aria-pressed', String(!isOnline));
        onlineButton.className = `payment-method-toggle p-4 text-left transition-colors ${isOnline ? 'border-2 border-vintage-gold bg-vintage-gold/5' : 'border border-black/15 bg-white hover:border-vintage-gold/60'}`;
        codButton.className = `payment-method-toggle p-4 text-left transition-colors ${!isOnline ? 'border-2 border-vintage-gold bg-vintage-gold/5' : 'border border-black/15 bg-white hover:border-vintage-gold/60'}`;
        buttonText.textContent = isOnline ? onlinePaymentButtonLabel : cashOnDeliveryButtonLabel;
        submitHelp.textContent = isOnline ? 'După confirmarea plății, veți vedea imediat starea comenzii.' : 'Comanda va fi înregistrată fără debitarea unui card.';
        clearErrors();
    }

    onlineButton.addEventListener('click', () => selectMethod('online'));
    ...
    function setLoading(loading) {
        ...
        onlineButton.disabled = loading;
        codButton.disabled = loading;
        spinner.classList.toggle('hidden', !loading);
        buttonText.textContent = loading
            ? (checkoutMethod === 'online' ? 'Se procesează plata…' : 'Se înregistrează comanda…')
            : (checkoutMethod === 'online' ? onlinePaymentButtonLabel : cashOnDeliveryButtonLabel);
    }

    function paymentErrorMessage(error) {
        const messages = {
            card_declined: 'Cardul a fost refuzat. Încercați un alt card sau contactați banca.', expired_card: 'Cardul este expirat. Folosiți un alt card.',
            incorrect_cvc: 'Codul de securitate al cardului este incorect.', processing_error: 'Plata nu a putut fi procesată momentan. Încercați din nou.',
            incomplete_number: 'Numărul cardului este incomplet.', incomplete_expiry: 'Data expirării este incompletă.', incomplete_cvc: 'Codul de securitate este incomplet.',
        };
        return messages[error?.code] || error?.message || 'Operațiunea nu a putut fi finalizată. Verificați datele și încercați din nou.';
    }
    ...
    async function createPaymentIntentAfterAcceptance() {
        const response = await fetch({{ Illuminate\Support\Js::from(route('checkout.accept-terms')) }}, {
            method: 'POST',
            headers: {Accept: 'application/json', 'Content-Type': 'application/json', 'X-CSRF-TOKEN': {{ Illuminate\Support\Js::from(csrf_token()) }}},
            credentials: 'same-origin',
            body: JSON.stringify({order_token: {{ Illuminate\Support\Js::from($orderToken) }}, accept_terms: termsInput.checked, acknowledge_privacy: privacyInput.checked}),
        });
    ...
    async function placeCashOnDelivery() {
        ...
        if (!addressResult.complete) throw new Error('Completați toate câmpurile obligatorii din adresa de livrare.');
        ...
        const response = await fetch({{ Illuminate\Support\Js::from(route('checkout.cash-on-delivery')) }}, {
            ...
            credentials: 'same-origin',
            body: JSON.stringify({
                order_token: {{ Illuminate\Support\Js::from($orderToken) }}, accept_terms: termsInput.checked, acknowledge_privacy: privacyInput.checked,
                customer: {email: emailInput.value.trim(), name: value.name || '', phone: value.phone || '', address: value.address || {}},
            }),
        });
        ...
        if (!response.ok) {
            ...
            throw new Error(firstValidationError || payload.message || 'Comanda ramburs nu a putut fi înregistrată.');
        }
        if (!payload.redirect_url) throw new Error('Comanda a fost înregistrată, dar confirmarea nu a putut fi deschisă.');
