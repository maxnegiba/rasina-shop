...
<div class="bg-ivory min-h-[70vh] py-20 md:py-28">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <div class="inline-flex w-20 h-20 rounded-full items-center justify-center mb-7 border {{ $isPaid ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : ($isPending ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-red-50 border-red-200 text-red-700') }}">
            @if($isPaid)
        ...
        <span class="block text-vintage-gold tracking-[0.22em] text-[10px] font-semibold uppercase mb-3">
            {{ $isPaid ? 'Comandă confirmată' : ($isPending ? 'Plată în curs de confirmare' : 'Plată nefinalizată') }}
        </span>
        <h1 class="font-serif text-4xl md:text-5xl text-dark-brown mb-5">
            {{ $isPaid ? 'Vă mulțumim pentru comandă!' : ($isPending ? 'Verificăm plata' : 'Plata nu a fost confirmată') }}
        </h1>
        ...
        @elseif($isFailed)
            <p class="text-dark-brown/65 font-light leading-relaxed">
                Nu a fost confirmată nicio debitare. Produsele rămân în coș, iar plata poate fi încercată din nou cu aceeași metodă sau cu o altă metodă disponibilă.
            </p>
        @else
            <p class="text-dark-brown/65 font-light leading-relaxed">
                Linkul de confirmare este incomplet sau sesiunea nu mai este validă. Verificați emailul de confirmare înainte de a iniția o nouă plată.
            </p>
        @endif

        <div class="mt-10 flex flex-col sm:flex-row justify-center gap-3">
            @if($isFailed)
                <a href="{{ route('checkout.index') }}" class="bg-dark-brown text-white px-8 py-4 uppercase tracking-[0.16em] text-[10px] font-semibold hover:bg-vintage-gold transition-colors">Încearcă plata din nou</a>
                <form method="POST" action="{{ route('checkout.cancel') }}">
                    @csrf
                    <button type="submit" class="w-full border border-dark-brown text-dark-brown px-8 py-4 uppercase tracking-[0.16em] text-[10px] font-semibold hover:bg-dark-brown hover:text-white transition-colors">Revino la coș</button>
                </form>
            @else
