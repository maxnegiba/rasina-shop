...
<div class="bg-ivory min-h-[70vh] py-20 md:py-28">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <div class="inline-flex w-20 h-20 rounded-full items-center justify-center mb-7 border {{ ($isPaid || $isCod) ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : ($isPending ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-red-50 border-red-200 text-red-700') }}">
            @if($isPaid || $isCod)
        ...
        <span class="block text-vintage-gold tracking-[0.22em] text-[10px] font-semibold uppercase mb-3">
            {{ $isPaid ? 'Comandă confirmată' : ($isCod ? 'Comandă înregistrată' : ($isPending ? 'Plată în curs de confirmare' : 'Plată nefinalizată')) }}
        </span>
        <h1 class="font-serif text-4xl md:text-5xl text-dark-brown mb-5">
            {{ ($isPaid || $isCod) ? 'Vă mulțumim pentru comandă!' : ($isPending ? 'Verificăm plata' : 'Plata nu a fost confirmată') }}
        </h1>
        <div class="w-14 h-px bg-vintage-gold mx-auto mb-7"></div>

        @if($isPaid)
            <p class="text-dark-brown/65 font-light leading-relaxed">Plata a fost validată în siguranță. Confirmarea comenzii și documentul proforma au fost trimise la adresa de email folosită la plată.</p>
        @elseif($isCod)
            <p class="text-dark-brown/65 font-light leading-relaxed">Comanda a fost înregistrată cu <strong>plata ramburs</strong>. Nu a fost debitat niciun card. Veți achita suma curierului în momentul livrării.</p>
        @elseif($isPending)
            <p class="text-dark-brown/65 font-light leading-relaxed">Confirmarea poate dura câteva momente. Nu inițiați o plată nouă și nu apăsați din nou butonul de plată.</p>
        @elseif($isFailed)
            <p class="text-dark-brown/65 font-light leading-relaxed">Nu a fost confirmată nicio debitare. Produsele rămân în coș, iar plata poate fi încercată din nou.</p>
        @else
            <p class="text-dark-brown/65 font-light leading-relaxed">Linkul de confirmare este incomplet sau sesiunea nu mai este validă.</p>
        @endif

        @if(($isPaid || $isCod) && $order)
            <div class="mt-8 bg-white border border-black/5 p-5 text-left shadow-sm">
                <dl class="grid sm:grid-cols-2 gap-4 text-sm">
                    ...
                    <div><dt class="text-[10px] uppercase tracking-[0.12em] text-dark-brown/45">{{ $isCod ? 'Total de plată la livrare' : 'Total achitat' }}</dt><dd class="mt-1 font-semibold text-dark-brown">{{ number_format($order->total_amount, 2, ',', '.') }} RON</dd></div>
                </dl>
            </div>
        ...
        <div class="mt-10 flex flex-col sm:flex-row justify-center gap-3">
            @if($isFailed)
                <a href="{{ route('checkout.index') }}" class="bg-dark-brown text-white px-8 py-4 uppercase tracking-[0.16em] text-[10px] font-semibold hover:bg-vintage-gold transition-colors">Încearcă din nou</a>
...
(() => {
    ...
    if (refreshes < 4) { window.sessionStorage.setItem(key, String(refreshes + 1)); window.setTimeout(() => window.location.reload(), 3500); }
})();
</script>
@elseif($isPaid || $isCod)
<script>
try { window.sessionStorage.removeItem('mtd-payment-status-refreshes'); window.sessionStorage.removeItem('mtd-checkout-email'); } catch (_) {}
</script>
@endif
@endsection
