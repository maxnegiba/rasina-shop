...
<head>
    ...
    <meta name="p:domain_verify" content="7f3b36eeb44634f2c91b67046d99d844">
    @hasSection('seo_meta')
        @yield('seo_meta')
    @else
        {!! seo() !!}
...
<body class="bg-ivory text-dark-brown font-sans antialiased selection:bg-vintage-gold selection:text-white flex flex-col min-h-screen">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-24 items-center relative">
                    ...
                    <a href="{{ route('home') }}" aria-label="MTD ART — pagina principală" class="font-serif text-lg md:text-2xl tracking-[0.2em] uppercase text-dark-brown hover:text-vintage-gold transition-colors duration-300 flex items-center gap-2 md:gap-3">
                        <div class="relative h-24 w-24 md:w-32 flex items-center justify-center">
                            <img src="{{ $logo192 }}"
                                 ...
                                 fetchpriority="high"
                                 decoding="async"
                                 class="absolute top-1 md:top-2 h-24 md:h-32 w-auto object-contain transition-all duration-300">
                        </div>
                        <span>MTD</span>
                    </a>
                </div>
                ...
                <div class="flex items-center space-x-6">
                    <button id="cart-menu-btn" type="button" aria-label="Deschide coșul" aria-controls="cart-sidebar" class="text-dark-brown/80 hover:text-vintage-gold transition duration-300 relative group focus:outline-none focus-visible:ring-2 focus-visible:ring-vintage-gold focus-visible:ring-offset-2">
                        ...
                        @endphp
                        <span id="cart-count-badge" aria-hidden="true" class="absolute -top-1 -right-2 bg-vintage-gold text-white text-[9px] min-w-4 h-4 px-1 flex items-center justify-center rounded-full opacity-100 transition-opacity {{ $cartItemCount > 0 ? '' : 'hidden' }}">
                            {{ $cartItemCount }}
                        </span>
                    </button>
    ...
    <button id="floating-cart-btn" type="button" aria-label="Deschide coșul" class="fixed bottom-6 right-6 z-40 bg-dark-brown text-white p-4 rounded-full shadow-xl border border-vintage-gold/20 hover:bg-vintage-gold hover:-translate-y-1 transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-vintage-gold focus-visible:ring-offset-2 {{ $cartItemCount > 0 ? '' : 'hidden' }}">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
        </svg>
        <span id="floating-cart-count" aria-hidden="true" class="absolute -top-1 -right-1 bg-vintage-gold text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full border-2 border-ivory font-bold">
            {{ $cartItemCount }}
        </span>
    </button>
            ...
            <div class="border-t border-white/10 pt-8 flex flex-col md:flex-row justify-between items-center gap-6">
                ...
                <div class="flex items-center gap-4 opacity-70 grayscale hover:grayscale-0 transition-all duration-500">
                    <div class="flex items-center gap-1 text-[10px] text-white/70 uppercase tracking-[0.1em] mr-2">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>
                        Plăți Securizate
                    </div>
